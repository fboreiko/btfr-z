...
def _compute_rotation_curve_for_galaxy(galaxy, galaxy_idx, mass_model_catalog, samples, 
    ...
    r = np.asarray(selected_rows['R']) # kpc
    ...
    if return_all_data:
        Vdm_max = np.sqrt(np.nanmax(Vdm * np.abs(Vdm), axis=1)) # km/s
...
def compute_AM_realization(nu, abundance_match, deconv, emulator, galaxy_sample, mass_model_catalog, 
    ...
    if return_all_data:
        # Convert back to Msun and apply selection mask
        Mstellar = Mstellar / 0.7 * selection_mask
        # Calculate gas mass and apply selection mask
        Mgas = samples['MH1'] * 1.33 * selection_mask
        Mbar *= selection_mask
        # Return the catalogs matched via abundance matching
        catalog_Mvir_matched = np.log10(halo_catalog['Mvir'])[mask_matched]
        return vc, Vdm_max, Mstellar, Mgas, Mbar, catalog_Mstar_matched, catalog_Mvir_matched
    else:
        return vc
...
def compute_likelihood(alpha, scatter, nu, abundance_match, emulator, galaxy_sample, mass_model_catalog, 
    ...
    if return_all_data:
        ...
        local_Mgas = np.array([result[3] for result in local_results])
        local_Mbar = np.array([result[4] for result in local_results])
        AMcatalog_Mstar = np.array([result[5] for result in local_results])
        AMcatalog_Mvir = np.array([result[6] for result in local_results])
    else:
        # When return_all_data=False, results are just the vc arrays
        local_vc = np.array([np.array(result) for result in local_results])
        local_vdm = None
        local_Mstellar = None
        local_Mgas = None
        local_Mbar = None
    ...
    if rank == 0:
        ...
        if return_all_data:
            ...
            gathered_Mgas = np.empty((n_am_reals, len(galaxy_sample), n_stellar_reals))
    ...
    if return_all_data:
        ...
        comm.Gather(local_Mgas, gathered_Mgas, root=0)
    ...
    if rank == 0:
        ...
        if return_all_data:
            ...
            Mgas_sims = np.log10(np.transpose(gathered_Mgas, (1, 0, 2)))
            Mgas_sims_flat = Mgas_sims.reshape(len(galaxy_sample), -1)
            Mbar_sims = np.log10(np.transpose(gathered_Mbar, (1, 0, 2)))
            Mbar_sims_flat = Mbar_sims.reshape(len(galaxy_sample), -1)
            return log_likelihood, inidividual_log_likelihoods, V_sims_mode, Vdm_sims_flat, \
                Mstellar_sims_flat, Mgas_sims_flat, Mbar_sims_flat, AMcatalog_Mstar, AMcatalog_Mvir
        else:
