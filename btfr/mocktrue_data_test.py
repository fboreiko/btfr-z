...
def compute_likelihood(alpha_value, scatter_value, nu_value, abundance_match, emulator, galaxy_sample, mass_model_catalog, 
    ...
    if rank == 0:
        ...
        V_obs_unlogged = np.array(mock_vels)
        V_obs_err_unlogged = np.array(mock_vels_err)
...
if __name__ == "__main__":
   ...
   # Load contra interpolators
    ...
    N_SAMPLES = 30  # must match the grid generation
    ...
    true_scatter = 0.1
    true_x = 0.6
    true_nu = np.linspace(-3.0, 3.0, 20)[5]
    ...
    if rank == 0:
        ...
        for i in range(NUM_TRUTHS):
            ...
            mock_vels_output = compute_AM_realization(abundance_match, deconv, true_nu, contra_interpolator, galaxy_list, 
                                ...
                                MH1_means, MH1_errors, dists, dists_err, n_stellar_reals=10)
    ...
    alpha_proxy_range = np.linspace(-np.pi / 2, np.pi / 2, 10)
    alpha_range = np.tan(alpha_proxy_range)
    scatter_range = np.linspace(0.01, 0.8, 20)
    x_range = np.linspace(0.5, 0.99, 20)
    nu_range = np.linspace(-1.0, 1.5, 40)

    N_STELLAR_REALS_MINX = int(N_STELLAR_REALS * (1 - np.max(x_range)) / (1 - np.min(x_range)))
    n_stellar_range = np.floor(np.linspace(N_STELLAR_REALS_MINX, N_STELLAR_REALS, 20)).astype(int)
    ...
    likelihood_grids = np.empty((num_truths_corrected, len(alpha_range), len(scatter_range), len(x_range), len(nu_range)))
        ...
        for i_nu, nu in enumerate(nu_range):
            ...
            for i_alpha, alpha in enumerate(alpha_range):
                ...
                for i_scatter, scatter in enumerate(scatter_range):
                    ...
                    likelihoods = compute_likelihood(alpha, scatter, nu, abundance_match, contra_interpolator, galaxy_list, mass_models, mock_vels_ensemble, 
    ...
    if rank == 0:
        ...
        if Vmax_shift_mode:
            np.save(f'likelihood_grid_{num_truths_corrected}mocktruths_{N_AM_REALS}am_{N_STELLAR_REALS}stellar_alphaproxy_{np.min(alpha_proxy_range)}_{np.max(alpha_proxy_range)}_scatter_{np.min(scatter_range)}_{np.max(scatter_range)}_x_{np.min(x_range)}_{np.max(x_range)}_nu_{np.min(nu_range)}_{np.max(nu_range)}_vmaxshift.npy', likelihood_grids)
        else:
